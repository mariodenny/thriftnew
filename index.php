...
<head>
    <style>
        ...
        /* Background keseluruhan kategori */
        .box_kategori1 {
            background-color: transparent;
        }

        /* Background dalam tiap kategori */
        .isi_kategori {
    ...
    <div class="width">
        ...
        <div class="flash_sale">
            ...
            <div class="box_iklan_flash_sale grid_terlaris">
                <?php
                $produk_terlaris = $server->query("SELECT * FROM `iklan` WHERE `tipe_iklan`='' AND `status`='' ORDER BY `id` DESC LIMIT 10");
                while ($produk_terlaris_data = mysqli_fetch_assoc($produk_terlaris)) {
                    $exp_gambar_pt = explode(',', $produk_terlaris_data['gambar']);
                    $terjual = mysqli_num_rows($server->query("SELECT * FROM `invoice` WHERE `id_iklan`='$produk_terlaris_data[id]'"));
        ...
        <div class="flash_sale">
            ...
            <div class="box_iklan_flash_sale grid_terlaris">
                <?php
                $produk_terlaris = $server->query("SELECT * FROM `iklan` WHERE `tipe_iklan`='' AND `status`='' ORDER BY `iklan`.`terjual` DESC LIMIT 10");
                while ($produk_terlaris_data = mysqli_fetch_assoc($produk_terlaris)) {
                    $exp_gambar_pt = explode(',', $produk_terlaris_data['gambar']);
                    $terjual = mysqli_num_rows($server->query("SELECT * FROM `invoice` WHERE `id_iklan`='$produk_terlaris_data[id]'"));
                ?>
    ...
    <script>
        $(document).ready(function() {
            $('.owl-carousel').owlCarousel({
                ...
                margin: 10,
                nav: true,
                autoplay: true, // Untuk slideshow otomatis
                autoplayTimeout: 3000, // Durasi tiap slide (dalam milidetik)
                autoplayHoverPause: true, // Berhenti ketika mouse diarahkan
                responsive: {
                    0: {
                        items: 1
                    },
